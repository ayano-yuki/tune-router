...
| `dev.json` | 調整・確認用データ |
| `test.json` | 固定評価用データ |
| `dataset2.json` | PR由来データを現行ラベルへ変換した追加データ |
| `train2.json` | `dataset2.json` の学習用分割 |
| `dev2.json` | `dataset2.json` の調整・確認用分割 |
| `test2_eval.json` | `dataset2.json` の固定評価用分割 |
| `test2.json` | 手動プロンプトリストを現行ラベルへ変換した評価データ |
